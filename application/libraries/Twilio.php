...
class Services_Twilio extends Services_Twilio_Resource {
	 ...
	 * :rtype: object
	 */
	public function retrieveData($path, $params = array(), $full_uri = false
	) {
	...
	private function _processResponse($response) {
		...
		if ($decoded === null) {
			...
			$status, 'Could not decode response body as JSON. ' .
		...
		throw new Services_Twilio_RestException(
		$status, isset($decoded->message) ? $decoded->message : '', isset($decoded->code) ? $decoded->code : null, isset($decoded->more_info) ? $decoded->more_info : null
		);
	}
